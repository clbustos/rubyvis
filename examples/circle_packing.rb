...
pack = vis.add(Rubyvis::Layout::Pack)
.nodes(pv.dom(files).root("Rubyvis").nodes())
.size(lambda {|d| d.node_value})

pack.node.add(Rubyvis::Dot)
.fill_style(lambda {|d| d.first_child ? "rgba(31, 119, 180, 0.25)" : "#ff7f0e"})
.title(lambda {|d| d.node_name.to_s + (d.first_child ? "" : ": " + format.format(d.node_value))})
    .line_width(1)

pack.node_label.add(Rubyvis::Label)
.visible(lambda {|d| !d.first_child})
.text(lambda {|d| d.node_name[0, Math.sqrt(d.node_value) / 10]})
